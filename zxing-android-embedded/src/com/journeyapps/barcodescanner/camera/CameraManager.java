...
import com.google.zxing.client.android.camera.CameraConfigurationUtilsEmbedded;
...
public final class CameraManager {
    ...
    private void setDesiredParameters(boolean safeMode) {
        ...
        CameraConfigurationUtilsEmbedded.setFocus(parameters, settings.getFocusMode(), safeMode);

        if (!safeMode) {
            CameraConfigurationUtilsEmbedded.setTorch(parameters, false);

            if (settings.isScanInverted()) {
                CameraConfigurationUtilsEmbedded.setInvertColor(parameters);
            }

            if (settings.isBarcodeSceneModeEnabled()) {
                CameraConfigurationUtilsEmbedded.setBarcodeSceneMode(parameters);
            }

            if (settings.isMeteringEnabled()) {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH_MR1) {
                    CameraConfigurationUtilsEmbedded.setVideoStabilization(parameters);
                    CameraConfigurationUtilsEmbedded.setFocusArea(parameters);
                    CameraConfigurationUtilsEmbedded.setMetering(parameters);
                }
            }
        ...
        if (Build.DEVICE.equals("glass-1")) {
            ...
            CameraConfigurationUtilsEmbedded.setBestPreviewFPS(parameters);
        }
        ...
        if (camera != null) {
            try {
                ...
                if (on != isOn) {
                    ...
                    Camera.Parameters parameters = camera.getParameters();
                    CameraConfigurationUtilsEmbedded.setTorch(parameters, on);
                    if (settings.isExposureEnabled()) {
                        CameraConfigurationUtilsEmbedded.setBestExposure(parameters, on);
                    }
                    camera.setParameters(parameters);
